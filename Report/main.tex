...
% Compile from this directory; upload main.tex and figures/ to Overleaf.
...
\vfill
{\color{greytext}\large Anthony Iemmolo, Marco Leonardi, Alessio Timofte\par}
\vfill
...
The following figures are exact copies of the saved plots. Smoothing uses up to 100 preceding episodes; reward bands are 95\% bootstrap confidence intervals for the rolling mean, obtained by resampling the episodes inside each window with replacement. They describe how precisely each window average is measured within one run, not how much the outcome would vary across seeds. Unless another window is stated, final training summaries use episodes 4,751--5,000, a 250-episode block, and therefore differ from plotted 100-episode endpoints. Training measurements include epsilon-greedy exploration and changing policies. Each final checkpoint was evaluated separately for one greedy episode with seed 1234 on its saved fixed task. Configuration records link each evaluation to its checkpoint; exact evaluation and figure paths are listed in \path{Report/figures/README.md}.
...
Collecting the final flag terminates on that same transition, with no additional action or step penalty. The 100-step limit is an external sampling cutoff rather than a task deadline; no remaining-time variable is required for that cutoff interpretation.
...
The SARSA loop selects the initial action before stepping. After a non-terminal transition it selects one next action, passes that action explicitly to \code{update\_SARSA}, and carries it forward for execution. This now follows the standard SARSA trajectory; the former separate resampling inside the update has been removed. The cliff agent uses the same training logic.

Both updates mask the bootstrap only on \code{terminated}. A truncated transition retains its continuation value, while either flag stops the roll-out. All three main environments use this same external-cutoff interpretation, as discussed in Section~\ref{sec:limits}.

\subsubsection{Training results and greedy evaluation}

\trainingfigure{GridFlag_training_curves_qlearning_bootstrap.png}
 {Q-learning: episode return, length, mean absolute TD error and epsilon.}
 {Flag Q-learning, 5,000 episodes with seed 0. Return and length improve as training proceeds and exploration declines. The shaded band is a 95\% bootstrap confidence interval for the rolling mean.}
{fig:flag-q}
\trainingfigure{GridFlag_training_curves_sarsa_bootstrap.png}
...
\trainingfigure{GridFlag_outcomes_qlearning_bootstrap.png}
 ...
 {fig:flag-q-outcomes}
\trainingfigure{GridFlag_outcomes_sarsa_bootstrap.png}
...
\end{equation}
where $d_t$ is goal distance and $\phi_t\in[-\pi,\pi)$ is the relative goal bearing. Absolute $(x,y)$ coordinates are not included. The action space is two-dimensional and continous.
...
\end{equation}
Terminal rewards replace dense shaping on that step. Reaching the sampling limit without a terminal event produces truncation and the ordinary dense reward: there is no additional timeout penalty.
...
All three agents are model-free, off-policy actor--critic methods implemented in PyTorch. An actor selects actions and one or more critics estimate action values. Replay buffers retain raw observations, actions, rewards, next observations, and a terminal mask; uniformly sampled minibatches reuse transitions collected by earlier policies.
...
Adam optimizers update network parameters. Actor and critic gradients are clipped to a global norm of 1.0 before each optimizer step. Target networks start as copies of the online networks and are updated by
\begin{equation}
...
The first 5,000 environment steps use uniformly random actions, and gradient updates begin after this warmup. Only true termination (goal or collision) zeroes the bootstrap term in the targets; reaching the step limit ends the rollout but keeps the continuation value. During the actor update the critic's parameters are held fixed, so the actor learns only through the critic's action gradient.
...
The implementation uses the original paper's smoothing values, $\sigma_{\mathrm{target}}=0.2$ and $c=0.5$, and updates the actor every two critic updates. Target smoothing acts inside the Bellman target and is separate from exploration: unlike the original, which explores with uncorrelated Gaussian noise $\mathcal N(0,0.1)$, this agent uses the same OU exploration as DDPG. With one critic update per environment step, the actor and the three target networks are updated every second step.
...
\end{equation}
This is the log-temperature loss used in the code. The target critics are softly updated after each learning update. A stochastic policy and twin critics provide mechanisms for exploration and reduced overestimation. The temperature also need not decrease monotonically during learning.
...
\rowcolor{lightbg} Critic learning rate&$10^{-3}$&$3\cdot10^{-4}$&$3\cdot10^{-4}$\\
...
Critic updates per environment step after warmup&1&1&2\\
...
The selected artifacts are the latest completed training run for each algorithm in the supplied project. DDPG and SAC were created on 10 September 2026; TD3 was retrained with revised hyperparameters on 13 September 2026. Their directories beneath \path{artifacts/continuous/} are:
\begin{itemize}
 \item DDPG: \path{ddpg/20260910T154140101411Z_seed0_3d2d5e39/}.
 \item TD3: \path{td3/20260913T102310246485Z_seed0_5cf3e17f/}.
...
\rowcolor{lightbg} TD3&1,301,110&1,296,111&648,055\\
...
%Earlier runs used separate greedy demonstration videos reduced to at most 320 pixels wide. The current recorder preserves native resolution up to 800 pixels wide and uses filtered resizing for larger frames, avoiding the thin grid lines lost by the earlier pixel-skipping method. Cliff recordings retain their native $800\times250$ resolution. Recordings remain capped at 300 frames. Evaluation continues to termination or truncation after recording stops. Names such as \code{episode\_0.mp4} and \code{episode\_500.mp4} use zero-based training indices, and a demonstration follows the corresponding training episode's updates. Thus video 0 follows the first episode; CSV episode numbers and plot axes count completed episodes from 1. Notebook summaries retain each block of 500 completed episodes above the updating total progress bar.

\subsection{Training results and interpretation}
\label{sec:continuous-results}

The nine figures in this subsection come from the three training runs identified in Section~\ref{sec:protocol}. Their measurements are outcomes under exploration: DDPG and TD3 use OU noise after warmup, whereas SAC samples its stochastic policy. They are not greedy evaluation curves or repeated measurements of only the final checkpoint. Numerical claims below are recomputed from the corresponding episode CSVs.

The return, length and loss curves use a rolling window of 50 episodes, and the success-rate and distance curves use up to 100. Around each rolling mean, the shaded band is a 95\% bootstrap confidence interval: the episodes inside the window are resampled with replacement, and the band spans the central 95\% of the resampled means. It measures how precisely the window average is determined from a finite number of noisy episodes, covering the sampled layouts and the exploration noise of this run. It does not cover training randomness: initialization, the exploration sequence and the replay order were drawn once, so estimating their effect requires independent training seeds (Section~\ref{sec:limits}). Success curves average the binary goal-completion flags over 100-episode windows and carry the same interval.

The separate square distance plots show the distance at the end of every episode, including both successes and failures. The vertical axis spans the observed range, at most 11.6 m; the room diagonal is 14.1 m. Unless explicitly described as a plotted rolling value, the numerical summaries in the text and Table~\ref{tab:training-summaries} use the preceding \emph{250} episodes. This distinction matters when comparing a quoted number with the endpoint of a plotted 100-episode curve.

\subsubsection{DDPG: strong improvement with temporary regressions}

\trainingfigure{ddpg_training_curves_bootstrap.png}
 {DDPG episode return, episode length, critic loss and actor loss.}
 {Selected DDPG run over 5,000 completed episodes. Return and length show individual episodes and rolling means over 50 episodes. Shaded bands are 95\% bootstrap confidence intervals for the rolling means. Actor and critic losses are episode-averaged optimization diagnostics.}
 {fig:ddpg-training}
\trainingfigure{ddpg_success_rate_bootstrap.png}
 ...
 {fig:ddpg-outcomes}
\trainingfigure{ddpg_final_goal_distance_bootstrap.png}
 {DDPG final goal distance and its rolling mean.}
 {DDPG final goal distance in metres for all training episodes, with a rolling mean over up to 100 episodes and its 95\% bootstrap confidence interval. The vertical axis spans the full range of observed distances (maximum 11.5 m).}
 {fig:ddpg-distance}
...
\subsubsection{TD3: improved navigation after hyperparameter tuning}

Following an initial TD3 run, the critic learning rate was reduced from $10^{-3}$ to $3\cdot10^{-4}$ and critic updates per environment step from two to one. A fresh 5,000-episode run retained seed 0, the task and the other recorded agent settings, raising final-block training success from 53.2\% to 88.8\%. The two changes were tested together, so this comparison does not isolate their individual effects.

\trainingfigure{td3_training_curves_bootstrap.png}
 {TD3 episode return, episode length, critic loss and actor loss.}
 {TD3 after tuning, over 5,000 completed episodes. Rolling return increases; episode length first rises, peaking near episode 400, then falls. Shaded bands are 95\% bootstrap confidence intervals for the rolling means; actor and critic losses are optimization diagnostics.}
 {fig:td3-training}
\trainingfigure{td3_success_rate_bootstrap.png}
 {TD3 rolling goal-completion rate.}
 {TD3 success over up to 100 recent training episodes. After early improvement, the rate is generally near 90\%, with temporary regressions. The final 100-episode value is 89\%; the final 250-episode fraction is 88.8\%.}
 {fig:td3-outcomes}
\trainingfigure{td3_final_goal_distance_bootstrap.png}
 {TD3 final goal distance and its rolling mean.}
 {TD3 final goal distance. The rolling mean falls toward approximately 1 m, while occasional failures end farther from the goal. The vertical axis spans the full range of observed distances (maximum 11.6 m).}
 {fig:td3-distance}

The revised run shows substantial learning. Mean return rises from $-89.74$ in episodes 1--250 to 79.52 in the window ending at episode 1,000, while success increases from 29.6\% to 79.2\% and mean final distance decreases from 4.85 m to 1.63 m. The window ending at episode 3,000 reaches mean return 141.79 and success 93.6\%. The curves are not monotonic: success drops to 78.8\% over episodes 4,501--4,750 before recovering in the final block.

The final 250 episodes have mean return 128.22, mean length 194.29 steps and mean final distance 1.00 m. They contain 222 successes, 13 collisions and 15 cutoffs, giving 88.8\% success and 6.0\% cutoffs. This supports much more reliable navigation than the earlier run, while retaining measurable failures. Over the final 100 episodes, mean return is 130.54 and success 89\%.

In the initial run the critic loss spiked to about 3,700 near episode 2,530; after tuning it stays below 63 from episode 500 onward. It still remains nonzero and increases in parts of training despite improved task performance. Actor loss also changes direction late in the run. Neither curve alone establishes value accuracy or convergence; the navigation outcomes and the separate greedy evaluations below provide the practical evidence.
\FloatBarrier

\subsubsection{SAC: rapid early improvement and adaptive temperature}

\trainingfigure{sac_training_curves_bootstrap.png}
 {SAC episode return, episode length, critic loss and actor loss.}
 {SAC run over 3,000 completed episodes. Return and length have rolling means with 95\% bootstrap confidence intervals. The losses are episode averages and need not decrease monotonically as the policy and targets change.}
 {fig:sac-training}
\trainingfigure{sac_success_rate_bootstrap.png}
 {SAC rolling goal-completion rate.}
 {SAC rolling success rate with its 95\% bootstrap confidence interval. The final 100-episode success rate is 79\%.}
 {fig:sac-outcomes}
\trainingfigure{sac_final_goal_distance_bootstrap.png}
 {SAC final goal distance and its rolling mean.}
 {SAC final distance for all training episodes, with a rolling mean over up to 100 episodes and a horizontal span of 3,000 episodes. The vertical axis spans the full range of observed distances (maximum 11.3 m).}
 {fig:sac-distance}

 Over episodes 1--250, mean return is $-83.62$, success is 38.4\%, and mean final distance is 4.40 m. By the window ending at episode 1,000, mean return reaches 138.67, success is 90.4\%, mean length is 148.31 steps and mean final distance is 0.93 m. The pronounced rise in success around episodes 450--700 accompanies a reduction in both distance and episode length. This is faster early improvement on the episode axis than in the DDPG and TD3 runs (90.4\% success over the 250 episodes ending at episode 1,000, against 65.2\% and 79.2\%), but different horizons, networks and update budgets prevent treating it as a controlled efficiency comparison.
...
\rowcolor{lightbg} TD3&5,000&128.22&88.8\%&194.29&1.00\\
...
The tuned TD3 run has the highest final-block training return and success fraction among these selected runs, while DDPG has the shortest mean episode length. At the common endpoint of 3,000 completed episodes, the preceding 250-episode mean returns are 88.17 for DDPG, 141.79 for TD3 and 106.71 for SAC, with success fractions 78.4\%, 93.6\% and 83.2\%, respectively. Even this common episode endpoint does not equalize environment transitions, gradient updates or tuning effort. The table and curves describe these configurations; a general algorithm ranking still requires controlled evaluation and independent training seeds.
\FloatBarrier
...
These outputs belong to the archived comparison notebook and must not be used to rank the newly trained checkpoints shown in Section~\ref{sec:continuous-results}. The current checkpoints are evaluated below on this same map and on ten sampled layouts; a broader ranking would additionally require held-out maps and independent training replications.
...
The records are the runs ending in \code{93ca9597} and \code{ce4ec2e7} beneath \path{artifacts/continuous/ddpg/}. They support useful greedy behavior on a small sample from the implemented scenario families, not broad generalization or a comparison across independent training seeds.

\subsubsection{Current TD3 evaluation}

The tuned TD3 checkpoint reaches the fixed-map goal in 127 steps, with return 167.09 and final distance 0.432 m. On ten mixture-generated layouts with seeds 1234--1243, it succeeds nine times, reaches the cutoff once and has no collisions. Across all ten episodes, including the failure, mean return is 135.17, mean length is 206.5 steps and mean final distance is 1.04 m. The remaining failure reaches the 1,000-step cutoff 6.36 m from the goal.

These completed records are the runs ending in \code{be0e9c73} and \code{dd1e83d8} beneath \path{artifacts/continuous/td3/}. Both explicitly reference the selected tuned checkpoint and disable exploration noise. They confirm successful greedy navigation on the fixed map and most sampled layouts, without establishing failure-free control or an algorithm ranking from this small sample.

\subsubsection{Current SAC evaluation}

The SAC checkpoint reaches the fixed-map goal in 118 steps, with return 171.51 and final distance 0.453 m. On ten mixture-generated layouts with seeds 1234--1243, it succeeds nine times, reaches the cutoff once and has no collisions. Across all ten episodes, including the failure, mean return is 135.10, mean length is 201.7 steps and mean final distance is 0.91 m. The remaining failure reaches the 1,000-step cutoff 5.02 m from the goal, on the same layout where DDPG and TD3 also reach the cutoff.

These completed records are the runs ending in \code{58c65ad1} and \code{1876b453} beneath \path{artifacts/continuous/sac/}. Both reference the selected SAC checkpoint and use the deterministic mean action, with the same 1,000-step cutoff as the other evaluations, although SAC was trained with a 1,200-step cutoff. They confirm successful greedy navigation on the fixed map and most sampled layouts, without establishing failure-free control or an algorithm ranking from this small sample.
...
\bottomrule
\end{tabularx}
\caption{Comparison of all five algorithms.}
...
The continuous environment extends the problem to sensor-based navigation with bounded velocity commands and a shaped reward. The selected DDPG, tuned TD3 and SAC runs reach final 250-episode training success fractions of 85.2\%, 88.8\% and 83.2\%, respectively, with remaining failures. Current evaluations of the three final checkpoints all reach the goal on the fixed map, and on ten sampled layouts DDPG solves eight while TD3 and SAC solve nine; the same layout defeats all three. These are outcomes of particular configurations and training budgets, not a universal ranking. The archived three-agent benchmark remains historical evidence, separate from the new training results. Broader claims require independent training replications on shared layouts.
