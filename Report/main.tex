...
\trainingfigure{GridFlag_training_curves.png}
 ...
 {Flag Q-learning, 5,000 episodes with seed 0. Return and length improve as training proceeds and exploration declines. The reward band is rolling mean $\pm$ two rolling standard deviations within this run.}
{fig:flag-q}
...
Following the initial TD3 run, the critic learning rate was reduced from $10^{-3}$ to $3\cdot10^{-4}$ and critic updates per environment step from two to one. A fresh 5,000-episode run retained seed 0, the task and the other recorded agent settings, raising final-block training success from 53.2\% to 88.8\%. The two changes were tested together, so this comparison does not isolate their individual effects.

\trainingfigure{td3_training_curves.png}
 {TD3 episode return, episode length, critic loss and actor loss.}
 {TD3 after tuning, over 5,000 completed episodes. Rolling return increases and episode length decreases, with remaining failed episodes. The reward band is rolling mean $\pm$ two rolling standard deviations within this run; actor and critic losses are optimization diagnostics.}
 {fig:td3-training}
...
\trainingfigure{td3_final_goal_distance.png}
 ...
 {TD3 final goal distance. The rolling mean falls toward approximately 1 m, while occasional failures end farther from the goal. Forty-four episodes exceed 10 m and are marked at the upper edge; their full values remain in the mean.}
 {fig:td3-distance}
...
The final 250 episodes have mean return 128.22, mean length 194.29 steps and mean final distance 1.00 m. They contain 222 successes, 13 collisions and 15 cutoffs, giving 88.8\% success and 6.0\% cutoffs. This supports much more reliable navigation than the earlier run, while retaining measurable failures. The shorter 100-episode endpoint has mean return 130.54 and success 89\%.

The earlier extreme mid-training critic-loss spike is absent from these plots, but critic loss remains nonzero and increases in parts of training despite improved task performance. Actor loss also changes direction late in the run. Neither curve alone establishes value accuracy or convergence; the navigation outcomes and the separate greedy evaluations below provide the practical evidence.
\FloatBarrier
...
These completed records are \path{20260913T131252423944Z_seed1234_be0e9c73/} and \path{20260913T131254673271Z_seed1234_dd1e83d8/} beneath \path{artifacts/continuous/td3/}. Both explicitly reference the selected tuned checkpoint and disable exploration noise. They confirm successful greedy navigation on the fixed map and most sampled layouts, without establishing failure-free control or an algorithm ranking from this small sample.
\FloatBarrier
