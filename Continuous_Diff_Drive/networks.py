...
def _init_sac_weights(module: nn.Module):
    """SAC-specific initialization kept separate from DDPG init_weights."""
    if isinstance(module, nn.Linear):
...
class GaussianActor(nn.Module):
    """
    Stochastic Gaussian actor used by SAC.

    The network predicts a Gaussian mean and log-standard-deviation, samples
    with the reparameterization trick, applies tanh squashing, and rescales the
    result to the environment action bounds.
    """
    ...
    def sample(self, obs: torch.Tensor, deterministic: bool = False):
        """
        Return a squashed action and its corrected log probability.

        For deterministic evaluation, the mean action is used and log_prob is
        returned as None.
        """
...
class DoubleCritic(nn.Module):
    """
    Twin-Q critic used by SAC.

    It returns Q1(s, a) and Q2(s, a). SAC uses the smaller value to reduce
    overestimation bias.
    """
    ...
    def q1(self, obs: torch.Tensor, action: torch.Tensor):
        xu = torch.cat([obs, action], dim=1)
